...
**Formal registration + legal agreements (`/gicn/register` → `POST /api/gicn/profile`, server-authoritative 422):** the adult account holder provides formal details (guardian: full legal name, E.164 phone, relationship-to-child, address; school: org name, authorised-contact role + email, phone, address, **designated safeguarding lead**) and accepts a **versioned agreement bundle** (`lib/policies/gicnAgreements.ts`, `gicn-agreements-v1`): Programme Participation Terms, Privacy & Data-Protection (NDPA 2023), Child Safeguarding & Code of Conduct, and **Liability Waiver & Indemnity**. Profile + a tamper-evident `GicnAgreementAcceptance` row (IP/UA + SHA-256 hash via `lib/gicnAgreementHash.ts`) are written in one `$transaction`; re-signing is only required when the current bundle version isn't already accepted. Audited `gicn_agreement_accepted`; RLS-enabled. ⚠️ The agreement prose (esp. indemnity) is a default that needs Nigerian-counsel review and cannot waive a minor's statutory rights. `Participant` also gains optional `emergencyContact{Name,Phone}`.

**Prisma models:** `GicnProfile` (1:1 with `User`; + formal-registration columns), `GicnAgreementAcceptance` (tamper-evident agreement acceptance), `Participant` (+ `emergencyContact{Name,Phone}`), `Program` (string status `DRAFT|OPEN|CLOSED|COMPLETED`, string `type` from `PROGRAM_TYPES`), `ProgramRegistration` (`@@unique([participantId, programId])`, status `PENDING|CONFIRMED|WAITLISTED|CANCELLED`, unique `checkInCode`, `checkedInAt`), `Sponsorship` (string status `pending|paid|refunded|cancelled`), `ScholarshipAward` (encrypted `ninEncrypted`). Const-unions + helpers in `lib/gicn.ts`; bulk parse/validate in `lib/gicnRegistrationSchema.ts` (mirrors `lib/classificationSchema.ts`); **Paystack** payment gateway in `lib/payments.ts` (NGN/kobo; init + verify + HMAC-SHA512 signature) with DB-confirm orchestration in `lib/sponsorship.ts`.
...
**Scholarship Review Board (`/admin/gicn/scholarships`):** full lifecycle on `ScholarshipAward` (status `applied|under_review|awarded|rejected|onboarding|active|suspended|completed|terminated|withdrawn`, lifecycle source-of-truth + transitions in `lib/scholarship.ts`, decision orchestration in `lib/scholarshipDecide.ts`). Board nominates/reviews/awards/activates/suspends/etc. via `/api/admin/gicn/scholarships[/[id]/decide]` (perm **`scholarship.review`**); per-scholar **monitored profile** = compliance conditions, per-term academic records, manual **disbursement ledger** (perm **`scholarship.disburse`**), document vault, and a review timeline. **NIN + full payout account are encrypted at rest** (`encryptSecret`) and **never** returned by any list/detail API (only `hasNin`/`payoutAccountLast4`); the `…/[id]/reveal` endpoint requires `scholarship.disburse` + **`verifyStepUp`** + audit. New RBAC perms `scholarship.review` + `scholarship.disburse` (2FA-required; held by `owner` + `gicn_manager`; enterprise `manager` excluded). Models: `ScholarshipReview`, `ScholarshipCondition`, `ScholarshipAcademicRecord`, `ScholarshipDisbursement`, `ScholarshipDocument` (all RLS-enabled, cascade with the award). **Guardian flow (`app/gicn/(account)/scholarships/`, client-auth, ownership-enforced):** apply (own child + a `SCHOLARSHIP`-type programme → status `applied`), onboard an awarded scholarship (bank + NIN **encrypted**, accept conditions → `onboarding_submit`), upload documents (authenticated, `isAllowedUpload`-validated — not the public `/api/upload`), and a read-only monitored profile. Lifecycle emails via `lib/email.ts` (`sendScholarshipApplicationReceived/Awarded/Activated/Suspended`), fired from the decide route's `notify` payload. **Monitoring nudges** (`lib/scholarshipReminders.ts`, daily GH-Actions cron `/api/cron/gicn/scholarship-reminders`): renewal reminders to guardians (`renewalDueAt` within 14 days) + at-risk/breach nudges to the board — idempotent via the AuditLog.
...
A per-section role **hierarchy**, enforced **server-side, deny-by-default** via `requireAdmin(req, perm)` in `lib/rbac.ts`. UI hiding is UX only — the API is the source of truth. **Every new `/api/admin/*` mutation/sensitive route MUST call `requireAdmin` with the right `Permission`.**
- **Roles (8):** `owner` = `*` (total control: every section + app-global admin). **Recovery** tiers: `recovery_senior_manager` (full Recovery — incl. `pii.export`, `referrals.payout`, `webhooks.manage`, `retention.purge`) > `recovery_lead_manager` (`cases.read/write`, `pii.download`, `referrals.read`, `ops.email_test`) > `manager` (base — `cases.read/write` only); plus `viewer` (`cases.read` only). **GICN** tiers: `gicn_senior_manager` (full GICN — incl. `scholarship.disburse` + NIN reveal) > `gicn_lead_manager` (`gicn.manage/checkin` + `scholarship.review`) > `gicn_manager` (base — `gicn.manage/checkin`). **`users.manage` + `audit.purge` are owner-only.** Recovery and GICN are mutually isolated; **only `owner` spans both**. `normalizeRole()` maps any unknown/legacy-unmigrated value → `viewer` (least privilege); new admins **default to `viewer`**; bootstrap user is `owner`. (The role migration `20260611120000_admin_role_tiers` remaps legacy `manager`→`recovery_lead_manager`, `gicn_manager`→`gicn_lead_manager`.)
...
- **Retry & DLQ:** every delivery is a `WebhookDelivery` row. Backoff: 1m → 5m → 30m → 2h → 12h, then `status="dead"` at 5 attempts. Admin can manually retry a dead delivery from the deliveries panel.
- **Prompt retries via QStash (LIVE):** each non-dead failed attempt publishes a delayed QStash message (`lib/qstash.ts:scheduleWebhookRetry`) so the retry fires at its exact backoff time, not on the daily cron. QStash calls `/api/qstash/webhook-retry`, which **verifies the `Upstash-Signature`** (`verifyQstashSignature`) then runs `processRetries()` in `after()`. Graceful no-op if QStash env is unset; **`/api/cron/webhooks/retry` stays as the backstop.** ⚠️ **The QStash token is region-pinned** — `QSTASH_URL` MUST point at the matching region ingress (currently `https://qstash-us-east-1.upstash.io`); the SDK's default global endpoint rejects a region token (`user not found in this region`). Env: `QSTASH_TOKEN`, `QSTASH_CURRENT_SIGNING_KEY`, `QSTASH_NEXT_SIGNING_KEY`, `QSTASH_URL`.
...
| `/api/cron/classify/process` | daily (backstop) | Drains pending `StaffClassification` rows (25/chunk). **Self-continuing:** each chunk re-triggers the next via `lib/bulkClassify.ts:drainAndContinue`/`kickDrain` (this endpoint schedules the chunk in `after()` + returns immediately), so a large batch finishes in minutes instead of 25 rows/day. Rows process sequentially (AI naturally rate-limited). Cron is the backstop. |
