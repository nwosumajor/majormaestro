...
export default function LeadMagnet() {
  ...
  return (
    <div className="rounded-2xl border border-ink-600 bg-gradient-to-br from-ink to-slate-900 p-8 shadow-xl">
        ...
        <div className="mt-8 lg:mt-0">
          {success ? (
            <div className="rounded-2xl border border-emerald-400/30 bg-emerald-500/10 p-8 text-center">
              ...
              <p className="mt-1 text-sm text-slate-300">Check your inbox. The guide is on its way to <span className="font-semibold text-white">{email}</span>.</p>
              <p className="mt-4 text-xs text-slate-400">Not in your inbox? Check your spam folder or contact forensics@majormaestro.com</p>
            </div>
          ) : (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
              <p className="mb-1 text-sm font-bold text-white">Get the Free Guide</p>
              <p className="mb-5 text-xs text-slate-400">No spam. One email with the guide. Unsubscribe any time.</p>
              <form onSubmit={handleSubmit} className="space-y-3">
                <div>
                  <label className="mb-1.5 block text-xs font-semibold text-slate-300">Company Name <span className="font-normal text-slate-400">(optional)</span></label>
                  <input
                    type="text"
                    value={companyName}
                    onChange={(e) => setCompanyName(e.target.value)}
                    placeholder="e.g. Acme Industries Ltd"
                    className="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-2.5 text-sm text-white placeholder:text-slate-500 focus:border-emerald-400 focus:bg-white/15 focus:outline-none transition"
                  />
                </div>
                <div>
                  <label className="mb-1.5 block text-xs font-semibold text-slate-300">Official Email Address <span className="text-red-400">*</span></label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-white/10 bg-white/10 py-2.5 pl-9 pr-4 text-sm text-white placeholder:text-slate-500 focus:border-emerald-400 focus:bg-white/15 focus:outline-none transition"
                    />
                  </div>
                </div>
              ...
              <p className="mt-3 text-center text-xs text-slate-500">Your data is protected under NDPA 2023. We never share or sell contact information.</p>
            </div>
          )}
        </div>
      </div>
